...
| 【10】想定問題 | **試験範囲内のときのみ**。CCAR-F 形式の4択を1問、英語で出題し、その出題文を教材として解説する |

【7】と【9】を Academy 限定にしているのは、公式ドキュメント自体が一次資料である場合、そこから別のリンクへ誘導する価値が薄いため。Academy は概念説明なので、実装ドキュメントや動くスキルへの橋渡しに意味がある。

【10】想定問題は、Scenario・Question・選択肢A〜Dをすべて英語で出題したうえで、正解と誤答の理由（日本語）を示し、さらに**出題文そのものに対して**【スラッシュリーディング】【英語の型と語法】【語彙・語法メモ】【全訳】を後置する。1回の解釈で「本文の読解 → 英語での出題 → 出題文の読解」という二重の英語処理が生じる。

## 参照ファイル

SKILL.md 本体を軽く保つため、頻繁には参照しない情報は `references/` に分離してある。必要になった時点で読み込む段階的開示の構成。

| ファイル | 用途 |
|---|---|
| `hornby-patterns.md` | VP1〜VP25・NP1〜NP3・AP1〜AP3 の全型定義。【3】で型番号を書く前に必ず参照する |
| `hornby-tenses.md` | 12時制の定義と相の意味。動詞 write の活用変化を併記 |
| `hornby-noun-modifiers.md` | §3.81〜§3.84（名詞の形容詞的用法・分詞形容詞）。v3.5 以降は解説対象から外したが、後戻りできるよう保持 |
| `ccar-f-blueprint.md` | 5ドメイン・配点比率・タスクステートメント・6シナリオ。【8】で対応づける際に参照 |
| `ccar-f-keywords.md` | 公式 Exam Guide §17 由来の技術用語一覧、In-Scope / Out-of-Scope 論点、公式が名指しするアンチパターン14項目、判断の3軸 |
| `question-design.md` | 【10】用。公式サンプル問題12問の分析、英語出題の定型表現、誤答の型、作問チェックリスト |
| `anthropic-skills-catalog.md` | 【9】用。`anthropics/skills` 全17ディレクトリの概要・効く Domain・確認済み URL |
| `changelog-archive.md` | v9.0 以前の変更履歴。当時の番号体系のまま保存 |
...
**読解を演習に変換する（v10.0 → v10.3）** — CCAR-F は暗記ではなく判断を問う試験だが、読んで理解するだけでは「選択肢の中から選ぶ」という本番の行為を一度も練習できない。そこで【10】で毎回1問だけ4択を作る。自作問題が公式とずれないよう、必ず公式サンプル問題の判断3軸（確実性・根本原因・比例性）のいずれかを軸に据えることを必須制約にした。誤答も「一見もっともらしいが軸を外している」型を使う。

出題形式は二段階で改めた。v10.2 で**出題を日本語から英語に切り替えた**（本番は英語であり、日本語で判断だけ練習しても「英語の長いシナリオを読んで状況を把握する」負荷が再現できないため）。v10.3 では**解説対象を選択肢A〜Dまで広げ、全訳を追加した**。当初は分量を理由に選択肢を除外していたが、本番で最も時間をかけて読むのは選択肢であり、しかも誤答は語法の微妙な差（`keeping ~` の付帯状況が「既存を維持したまま」を含意するなど）で正誤が分かれる。実際に判断を要する箇所を教材から外していたことになる。
