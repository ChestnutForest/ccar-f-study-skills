...
5. **出題した英文を教材として解説する（v10.2・必須）。** 正解と誤答の理由（日本語）を書いた後に、**シナリオと設問の英文に対して**【スラッシュリーディング】【英語の型と語法】【語彙・語法メモ】の3ブロックを追記する。出題文は試験特有の言い回しの塊なので、それ自体が読解教材になる。選択肢A〜Dは分量が多いため、解説は**シナリオと設問に限る**（選択肢に重要な語法があれば語彙メモで触れる）。
